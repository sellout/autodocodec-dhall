...
toDhallTypeVia' = \case
  ...
  Autodo.ObjectOfCodec _ oc -> pure $ dhallObjectType oc
  -- no effect
  ...
  where
    dhallObjectType = toDhallObjectTypeVia
...
toDhallVia = flip go
  where
    ...
    goObject' a = \case
      Autodo.RequiredKeyCodec k c _ -> pure [(k, go a c)]
      Autodo.OptionalKeyCodec k c _ ->
        pure
          [ ( k,
              maybe
      ...
      Autodo.OptionalKeyWithDefaultCodec k c _ _ -> pure [(k, go a c)]
